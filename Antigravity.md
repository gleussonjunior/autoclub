...
- **Descrição**: Servidor MCP que fornece acesso completo à documentação de nós do n8n, propriedades e operações. Suporta 1.084+ nós (537 core + 547 community), 2.646 exemplos reais de configuração e 2.709+ templates de workflows.

### Skills do n8n
- **Repositório**: https://github.com/czlonkowski/n8n-skills
- **Descrição**: 7 skills complementares que ensinam como construir workflows n8n prontos para produção.
...
| `get_node` | Unified node information with modes: info (default), docs, search_properties, versions, compare, breaking, migrations |
| `validate_node` | Node validation: `mode: 'minimal'` (quick <100ms) or `mode: 'full'` (comprehensive with profiles) |
| `validate_workflow` | Complete workflow validation including AI Agent validation |
| `search_templates` | Unified template search: keyword, by_nodes, by_task, by_metadata |
...
| `n8n_get_workflow` | Unified workflow retrieval: full, details, structure, minimal |
...
- nodeType format: `nodes-base.*` vs `n8n-nodes-base.*`
...
- 5 patterns: webhook processing, HTTP API, database, AI, scheduled
...
### 7. n8n Code Python
Write Python code in n8n Code nodes with limitations awareness.
- Python data access: `_input`, `_json`, `_node`
- ⚠️ No external libraries (requests, pandas, numpy)
...
Without the `branch` parameter, both connections may end up on the same output!
...
// Response: "Found template by **Author** (@username). View at: https://n8n.io/workflows/..."
```
...
// STEP 3: Validate → Build → Validate Workflow → Deploy
```
